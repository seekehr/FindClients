...
**AI qualification** — opt-in, on your own Google Gemini key ([get one](https://aistudio.google.com/apikey)), entered on the Config page. Keyword scrapers are indiscriminate: searching "looking for a developer" finds the client who wants to hire *and* the developer announcing availability. The qualifier reads each lead against criteria you write in plain English and returns a 0–100 score plus a one-line reason. Gemini Flash only (`gemini-3.8-flash`, `3.7`, `3.6`, `3.5`, plus `gemini-3.5-flash-lite` and `gemini-3.1-flash-lite`; default 3.8 Flash), called over plain REST, 4 leads at a time. Flash-class throughout on purpose — one short judgment call per lead, billed per lead, is not work a Pro model does better. Failures record `error`, never `rejected` — a timeout is not evidence that a lead is bad. Already-reviewed leads are skipped, so turning qualification on later reviews the backlog.
